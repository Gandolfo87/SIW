...
(function() {
	var root = this;
	/*
	 * In <script> context, `this` is the window. In node context (browserify),
	 * `this` is the node global.
	 */
	var globalWindow = typeof global === 'undefined' ? root : root.window;
	var version = '0.5.1', settings = {}, mouse_is_down = false, keysDown = {}, key_map = {
		65 : 'Cl',
		87 : 'C#l',
		83 : 'Dl',
		69 : 'D#l',
	...
	var init = function(us) {
		...
		settings = {
			...
			startNote : user_settings.startNote || 'A1',
	 ...
	 * 
	 * @param {string}
	 *            note Musical note to convert into hertz.
	 * @return {number} Frequency of note in hertz.
	 */
	var getFrequencyOfNote = function(note) {
		var notes = [ 'A', 'A#', 'B', 'C', 'C#', 'D', 'D#', 'E', 'F', 'F#',
				'G', 'G#' ], key_number, octave;
	 ...
	 * 
	 * @param {element}
	 *            el DOM element to change colour of.
	 */
	 ...
	 * 
	 * @param {array}
	 *            notes_to_order Notes to be ordered.
	 ...
	 * 
	 * @param {element}
	 *            el Black key DOM element.
	 */
	var styleBlackKey = function(key) {
		var white_key_width = getWhiteKeyWidth(getTotalWhiteKeys()), black_key_width = Math
				.floor(white_key_width / 2);

		key.el.style.backgroundColor = settings.blackKeyColour;
		key.el.style.border = '1px solid ' + settings.borderColour;
		key.el.style.position = 'absolute';
		key.el.style.left = Math.floor(((white_key_width + 1.45) * (key.noteNumber + 1))- (black_key_width / 2))+ 'px';
	 ...
	 * 
	 * @param {element}
	 *            keyboard Keyboard container DOM element.
	 */
	...
	var getTotalBlackKeys = function() {

		return settings.octaves * 5;
	};

	var createKeys = function() {
		var that = this, i, key, keys = [], note_counter = 0, octave_counter = settings.startOctave, total_white_keys = getTotalWhiteKeys(),total_black_keys = getTotalBlackKeys();
						...
						.forEach(function(note, index) {
							if (note === that.whiteNotes[note_counter]) {
								key = createKey({
									...
									octave : octave_counter,
									width : document
											.getElementById("quintaRiga").clientWidth/ total_black_keys,
	 ...
	 * 
	 * @param {object}
	 *            key The keyboard event of the currently pressed key.
	 * @param {callback}
	 *            callback The user's noteDown function.
	...
	var keyboardDown = function(key, callback) {
		...
		if (typeof key_map[key.keyCode] !== 'undefined') {
			...
			console.log("sono il tasto vero", key);
	 ...
	 * 
	 * @param {element}
	 *            key The DOM element of the key that was released.
	 * @param {callback}
	 *            callback The user's noteDown function.
	 * @return {boolean} true if it was a key (combo) used by qwerty-hancock
	 */
	 ...
	 * 
	 * @param {KeyboardEvent}
	 *            The keydown event of a pressed key
	 */
