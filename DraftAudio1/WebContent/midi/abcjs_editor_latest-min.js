...
function(glob, factory) {
    ...
    var toHex = function(color) {
            if (R.vml) {
                ...
                i.title = "Raphaël Colour Picker", i.style.display = "none", g.doc.body.appendChild(i), toHex = cacher(function(color) {
    ...
    var pathDimensions = R.pathBBox = function(path) {
        ...
        pathToAbsolute = R._pathToAbsolute = function(pathArray) {
            ...
            if (R.is(pathArray, array) && R.is(pathArray && pathArray[0], array) || (pathArray = R.parsePathString(pathArray)), !pathArray || !pathArray.length) return [
                ["M", 0, 0]
            ];
            var res = [],
                x = 0,
        ...
        function() {
            if (R.svg) {
                ...
                Element.prototype = elproto, elproto.constructor = Element, R._engine.path = function(pathString, SVG) {
                    ...
                    return cx = null == cx ? bbox.x + bbox.width / 2 : cx, cy = null == cy ? bbox.y + bbox.height / 2 : cy, this.transform(this._.transform.concat([
                        ["s", sx, sy, cx, cy]
                    ])), this
                }, elproto.translate = function(dx, dy) {
                    return this.removed ? this : (dx = Str(dx).split(separator), dx.length - 1 && (dy = toFloat(dx[1])), dx = toFloat(dx[0]) || 0, dy = +dy || 0, this.transform(this._.transform.concat([
                        ["t", dx, dy]
                    ])), this)
                    ...
                    this.bottom = this.top = null, (this.desc = $("desc")).appendChild(R._g.doc.createTextNode("Created with Raphaël " + R.version)), c.appendChild(this.desc), c.appendChild(this.defs = $("defs"))
        ...
        function() {
            if (R.vml) {
                ...
                Element.prototype = elproto, elproto.constructor = Element, elproto.transform = function(tstr) {
                    ...
                    return this.removed ? this : (dx = Str(dx).split(separator), dx.length - 1 && (dy = toFloat(dx[1])), dx = toFloat(dx[0]) || 0, dy = +dy || 0, this._.bbox && (this._.bbox.x += dx, this._.bbox.y += dy), this.transform(this._.transform.concat([
                        ["t", dx, dy]
                    ])), this)
                }, elproto.scale = function(sx, sy, cx, cy) {
                    if (this.removed) return this;
    ...
    function() {
        ...
        ABCJS.startAnimation = function(paper, tune, options) {
            ...
            var startTime, initialWait = 2700,
                interval = 11,
                distance = 1,
            ...
            currentMargin = 0, options.scrollVertical && setMargin(0), startTime = new Date, startTime = startTime.getTime(), processNext()
        ...
        window.addEventListener("resize", resizeOuter), window.addEventListener("orientationChange", resizeOuter), ABCJS.renderAbc = function(output, abc, parserParams, engraverParams, renderParams) {
            ...
            function callback(div, tune, index) {
                ...
                if (midiParams.generateInline && (midiParams.animate || midiParams.listener)) {
                    ...
                    parent.abcjsTune = tune, parent.abcjsListener = midiParams.listener, parent.abcjsAnimate = midiParams.animate
                }
    ...
    function() {
        ...
        function midiJsListener(position) {
            ...
            if (position.duration > 0 && lastNow !== position.progress && (lastNow = position.progress, midiControl = find(document, "abcjs-midi-current"))) {
                ...
                midiControl.abcjsListener && midiControl.abcjsListener(midiControl, position)
            }
        ...
        function doReset(target, callback) {
            ...
            var parent = closest(target, "abcjs-inline-midi");
            deselectMidiControl(), loadMidi(parent, onSuccess)
        }
    ...
    function() {
        ...
        window.ABCJS.midi.create = function(abcTune, options) {
            ...
            var sequence = window.ABCJS.midi.sequence(abcTune, options),
                commands = window.ABCJS.midi.flatten(sequence),
    ...
    function() {
        ...
        function writeNote(elem) {
            ...
            if (elem.gracenotes) {
                ...
                graces = processGraceNotes(elem.gracenotes, stealFromDuration), bagpipes || (duration = writeGraceNotes(graces, stealFromCurrent, duration));
            }
            if (elem.pitches) {
                graces && bagpipes && (duration = writeGraceNotes(graces, !0, duration));
        ...
        function interpretChord(name) {
            if (0 !== name.length) {
                ...
                if (bass) {
                    ...
                    "b" === acc || "♭" === acc ? (bass--, bass2--, remaining = remaining.substring(1)) : "#" !== acc && "♯" !== acc || (bass++, bass2++, remaining = remaining.substring(1));
        ...
        var barAccidentals, accidentals, transpose, bagpipes, multiplier, tracks, startingTempo, instrument, channel, currentTrack, pitchesTied, lastNoteDurationPosition, meter, chordTrack, chordTrackFinished, currentChords, lastChord, barBeat, tempoChangeFactor = 1,
            normalBreakBetweenNotes = 1 / 128;
        window.ABCJS.midi.flatten = function(voices) {
            barAccidentals = [], accidentals = [0, 0, 0, 0, 0, 0, 0], transpose = 0, bagpipes = !1, multiplier = 1, tracks = [], startingTempo = void 0, tempoChangeFactor = 1, instrument = void 0, channel = void 0, currentTrack = void 0, pitchesTied = {}, lastNoteDurationPosition = -1, meter = void 0, chordTrack = [], chordTrackFinished = !1, currentChords = [], lastChord = void 0, barBeat = 0;
            for (var i = 0; i < voices.length; i++) {
                var voice = voices[i];
                ...
                for (var j = 0; j < voice.length; j++) {
                    ...
                    switch (element.el_type) {
                        ...
                        case "transpose":
                            ...
                            break;
                        case "bar":
                            chordTrack.length > 0 && (resolveChords(), currentChords = []), barBeat = 0, barAccidentals = [];
                            break;
                        case "bagpipes":
                            bagpipes = !0;
                            break;
                ...
                tracks.push(currentTrack), chordTrack.length > 0 && (chordTrackFinished = !0)
            }
            return chordTrack.length > 0 && tracks.push(chordTrack), {
                tempo: startingTempo,
    ...
    function() {
        ...
        window.ABCJS.midi.sequence = function(abctune, options) {
            ...
            for (var voices = [], startRepeatPlaceholder = [], skipEndingPlaceholder = [], i = 0; i < abctune.lines.length; i++) {
                ...
                if (line.staff)
                    for (var staves = line.staff, voiceNumber = 0, j = 0; j < staves.length; j++)
                        for (var staff = staves[j], k = 0; k < staff.voices.length; k++) {
                            ...
                            })), staff.meter && voices[voiceNumber].push(interpretMeter(staff.meter)), staff.clef && staff.clef.transpose && (staff.clef.el_type = "clef", voices[voiceNumber].push({
                                el_type: "transpose",
        ...
        function startNewLine() {
            ...
            if (multilineVars.partForNextLine.length && (params.part = multilineVars.partForNextLine), params.clef = multilineVars.currentVoice && void 0 !== multilineVars.staves[multilineVars.currentVoice.staffNum].clef ? window.ABCJS.parse.clone(multilineVars.staves[multilineVars.currentVoice.staffNum].clef) : window.ABCJS.parse.clone(multilineVars.clef), params.key = window.ABCJS.parse.parseKeyVoice.deepCopyKey(multilineVars.key), window.ABCJS.parse.parseKeyVoice.addPosToKey(params.clef, params.key), null !== multilineVars.meter ? (multilineVars.currentVoice ? (window.ABCJS.parse.each(multilineVars.staves, function(st) {
                ...
                staff.brace && (params.brace = staff.brace), staff.bracket && (params.bracket = staff.bracket), staff.connectBarLines && (params.connectBarLines = staff.connectBarLines), staff.name && (params.name = staff.name[multilineVars.currentVoice.index]), staff.subname && (params.subname = staff.subname[multilineVars.currentVoice.index]), multilineVars.currentVoice.stem && (params.stem = multilineVars.currentVoice.stem), multilineVars.currentVoice.scale && (params.scale = multilineVars.currentVoice.scale), multilineVars.currentVoice.style && (params.style = multilineVars.currentVoice.style)
            }
        ...
        var letter_to_chord = function(line, i) {
                if ('"' === line.charAt(i)) {
                    ...
                    } else chord[1] = chord[1].replace(/([ABCDEFG])b/g, "$1♭"), chord[1] = chord[1].replace(/([ABCDEFG])#/g, "$1♯"), chord[2] = "default";
                    return chord
                }
                return [0, ""]
            },
        ...
        window.ABCJS.parse.parseDirective.addDirective = function(str) {
            ...
            switch (cmd) {
                ...
                case "midi":
                    ...
                    if (midi.length > 0 && "=" === midi[0].token && midi.shift(), 0 === midi.length) warn("Expected midi command", restOfString, 0);
                    else {
                        var getNextMidiParam = function(midiToks) {
                            if (midiToks.length > 0) {
                                var t = midiToks.shift(),
                                    p = t.token;
                                return "number" === t.type && (p = t.intt), p
                            }
                            return null
                        };
                        void 0 === tune.formatting[cmd] && (tune.formatting[cmd] = {});
    ...
    function() {
        ...
        this.tokenize = function(line, start, end) {
            ...
            for (var i, tokens = []; start < end;) {
                if ('"' === line.charAt(start)) {
                    ...
                    for (i = start + 1; i < end && isLetter(line.charAt(i));) i++;
                    tokens.push({
                        type: "alpha",
        ...
        if (this.elemset = renderer.paper.set(), !this.invisible) {
            ...
            var spacing = ABCJS.write.spacing.STEP,
                ...
                move = function(dx, dy) {
                    dy = Math.round(dy / spacing) * spacing, this.translate(0, -this.dy),
                        this.dy = dy, this.translate(0, this.dy)
                },
                up = function() {
        ...
        ABCJS.write.getDuration = function(elem) {
            ...
            if (this.getElem().startBeam && !this.getElem().endBeam) {
                ...
                for (this.stemdir = dir; this.getElem();) {
                    ...
                    if (abselemset.push(abselem), beamelem.add(abselem), this.getElem().endBeam) break;
                    this.pos++
                }
                this.stemdir = oldDir, this.voice.addOther(beamelem)
            } else abselemset[0] = this.createNote(this.getElem());
            return abselemset
            ...
            if (!dontDraw && void 0 !== elem.gracenotes) {
                var gracescale = .6,
                    graceScaleStem = .7,
                    gracebeam = null;
                elem.gracenotes.length > 1 && (gracebeam = new ABCJS.write.BeamElem(this.stemHeight * graceScaleStem, "grace", this.isBagpipes), ABCJS.write.hint && gracebeam.setHint(), gracebeam.mainNote = abselem);
                var graceoffsets = [];
                ...
                for (i = 0; i < elem.gracenotes.length; i++) {
                    ...
                    0 !== i || this.isBagpipes || elem.rest && ("spacer" === elem.rest.type || "invisible" === elem.rest.type) || this.voice.addOther(new ABCJS.write.TieElem(grace, notehead, !1, !0, !1))
                }
                gracebeam && this.voice.addOther(gracebeam)
            }
            for (!dontDraw && elem.decoration && this.decoration.createDecoration(this.voice, elem.decoration, abselem.top, notehead ? notehead.w : 0, abselem, this.roomtaken, dir, abselem.bottom, elem.positioning, this.hasVocals), elem.barNumber && abselem.addChild(new ABCJS.write.RelativeElement(elem.barNumber, -10, 0, 0, {
                    type: "barNumber"
                })), i = elem.maxpitch; i > 11; i--) i % 2 !== 0 || elem.rest || abselem.addChild(new ABCJS.write.RelativeElement(null, -2, ABCJS.write.glyphs.getSymbolWidth(c) + 4, i, {
                type: "ledger"
            }));
            for (i = elem.minpitch; i < 1; i++) i % 2 !== 0 || elem.rest || abselem.addChild(new ABCJS.write.RelativeElement(null, -2, ABCJS.write.glyphs.getSymbolWidth(c) + 4, i, {
                type: "ledger"
            }));
            for (i = 0; i < additionalLedgers.length; i++) {
                var ofs = ABCJS.write.glyphs.getSymbolWidth(c);
                "down" === dir && (ofs = -ofs), abselem.addChild(new ABCJS.write.RelativeElement(null, ofs - 2, ABCJS.write.glyphs.getSymbolWidth(c) + 4, additionalLedgers[i], {
                    type: "ledger"
                }))
            }
            var chordMargin = 8;
            ...
            return elem.startTriplet && (this.triplet = new ABCJS.write.TripletElem(elem.startTriplet, notehead, null, !0), dontDraw || this.voice.addOther(this.triplet)), elem.endTriplet && this.triplet && (this.triplet.setCloseAnchor(notehead), this.triplet = null, this.tripletmultiplier = 1), abselem
        ...
        ABCJS.write.BeamElem.prototype.calcDir = function() {
            ...
            if (0 !== this.elems.length && !this.allrests) {
                ...
                var dy = calcDy(this.stemsUp, this.isgrace),
                    ...
                    yPos = calcYPos(this.total, this.elems.length, this.stemHeight, this.stemsUp, firstElement.abcelem.averagepitch, lastElement.abcelem.averagepitch, this.isflat, this.min, this.max, this.isgrace),
    ...
    function() {
        ...
        opt = opt || {}, this.x = 0, this.c = c, this.dx = dx, this.w = w, this.pitch = pitch, this.scalex = opt.scalex || 1, this.scaley = opt.scaley || 1, this.type = opt.type || "symbol", this.pitch2 = opt.pitch2, this.linewidth = opt.linewidth, this.klass = opt.klass, this.top = pitch, void 0 !== this.pitch2 && this.pitch2 > this.top && (this.top = this.pitch2), this.bottom = pitch, void 0 !== this.pitch2 && this.pitch2 < this.bottom && (this.bottom = this.pitch2), opt.thickness && (this.top += opt.thickness / 2, this.bottom -= opt.thickness / 2), opt.stemHeight && (opt.stemHeight > 0 ? this.top += opt.stemHeight : this.bottom += opt.stemHeight);
        ...
        x1 += 6, x2 += 4, pitch1 += above ? 1.5 : -1.5, pitch2 += above ? 1.5 : -1.5;
        var y1 = this.calcY(pitch1),
            ...
            uy = dy / norm,
            flatten = norm / 3.5,
            curve = (above ? -1 : 1) * Math.min(25, Math.max(4, flatten)),
        ...
        var ret = [];
        return c.length > 0 && ret.push(c), null !== this.lineNumber && ret.push("l" + this.lineNumber), null !== this.measureNumber && ret.push("m" + this.measureNumber),
            null !== this.voiceNumber && ret.push("v" + this.voiceNumber), ret.join(" ")
    ...
    function() {
        ...
        ABCJS.write.TempoElement = function(tempo, tuneNumber) {
            ...
            if (this.tempo.duration) {
                ...
                duration <= 1 / 32 ? (note = "noteheads.quarter", flag = "flags.u32nd", dot = 0) : duration <= 1 / 16 ? (note = "noteheads.quarter", flag = "flags.u16th", dot = 0) : duration <= 3 / 32 ? (note = "noteheads.quarter", flag = "flags.u32nd", dot = 1) : duration <= 1 / 8 ? (note = "noteheads.quarter", flag = "flags.u8th", dot = 0) : duration <= 3 / 16 ? (note = "noteheads.quarter", flag = "flags.u16th", dot = 1) : duration <= .25 ? (note = "noteheads.quarter", dot = 0) : duration <= 3 / 8 ? (note = "noteheads.quarter", flag = "flags.u8th", dot = 1) : duration <= .5 ? (note = "noteheads.half", dot = 0) : duration <= .75 ? (note = "noteheads.quarter", dot = 1) : duration <= 1 ? (note = "noteheads.whole", dot = 0) : duration <= 1.5 ? (note = "noteheads.half", dot = 1) : duration <= 2 ? (note = "noteheads.dbl", dot = 0) : duration <= 3 ? (note = "noteheads.whole", dot = 1) : (note = "noteheads.dbl", dot = 1);
        ...
        var klass;
        this.hint && (klass = "abcjs-hint"), renderer.drawArc(this.startX, this.endX, this.startY, this.endY, this.above, klass)
    }, window.ABCJS || (window.ABCJS = {}), window.ABCJS.write || (window.ABCJS.write = {}),
    function() {
        "use strict";
        ...
        ABCJS.write.TripletElem = function(number, anchor1) {
            ...
            this.anchor2 = anchor2
        }, ABCJS.write.TripletElem.prototype.setUpperAndLowerElements = function() {}, ABCJS.write.TripletElem.prototype.draw = function(renderer) {
            if (this.anchor1 && this.anchor2) {
                var xTextPos, yTextPos, hasBeam = this.anchor1.parent.beam && this.anchor1.parent.beam === this.anchor2.parent.beam;
                if (hasBeam) {
                    var beam = this.anchor1.parent.beam,
                        left = beam.isAbove() ? this.anchor1.x + this.anchor1.w : this.anchor1.x;
                    xTextPos = beam.xAtMidpoint(left, this.anchor2.x), yTextPos = beam.heightAtMidpoint(left, this.anchor2.x), yTextPos += beam.isAbove() ? 4 : -4
                } else {
                    var startNote = Math.max(this.anchor1.parent.top, 9) + 4,
                        endNote = Math.max(this.anchor2.parent.top, 9) + 4;
                    xTextPos = this.anchor1.x + (this.anchor2.x + this.anchor2.w - this.anchor1.x) / 2, yTextPos = startNote + (endNote - startNote) / 2, drawBracket(renderer, this.anchor1.x, startNote, this.anchor2.x + this.anchor2.w, endNote)
                }
                renderer.renderText(xTextPos, renderer.calcY(yTextPos), "" + this.number, "tripletfont", "triplet", "middle", !0)
            }
        }
        ...
        child instanceof ABCJS.write.BeamElem ? this.beams.push(child) : (this.otherchildren.push(child), this.setRange(child))
        ...
        return o.join("")
    }, window.ABCJS || (window.ABCJS = {}), window.ABCJS.edit || (window.ABCJS.edit = {}), window.ABCJS.edit.EditArea = function(textareaid) {
        this.textarea = document.getElementById(textareaid), this.initialText = this.textarea.value, this.isDragging = !1
    }, window.ABCJS.edit.EditArea.prototype.addSelectionListener = function(listener) {
        this.textarea.onmousemove = function(ev) {
            this.isDragging && listener.fireSelectionChanged()
        }
    }, window.ABCJS.edit.EditArea.prototype.addChangeListener = function(listener) {
        this.changelistener = listener, this.textarea.onkeyup = function() {
            listener.fireChanged()
        }, this.textarea.onmousedown = function() {
            this.isDragging = !0, listener.fireSelectionChanged()
        }, this.textarea.onmouseup = function() {
            this.isDragging = !1, listener.fireChanged()
        ...
        if (this.textarea.setSelectionRange) this.textarea.setSelectionRange(start, end);
            ...
            e.collapse(!0), e.moveEnd("character", end), e.moveStart("character", start), e.select()
        }
        this.textarea.focus()
    }, window.ABCJS.edit.EditArea.prototype.getString = function() {
        return this.textarea.value
    }, window.ABCJS.edit.EditArea.prototype.setString = function(str) {
        this.textarea.value = str, this.initialText = this.getString(), this.changelistener && this.changelistener.fireChanged()
    }, window.ABCJS.edit.EditArea.prototype.getElem = function() {
        return this.textarea
    }, window.ABCJS.Editor = function(editarea, params) {
        params.indicate_changed && (this.indicate_changed = !0), "string" == typeof editarea ? this.editarea = new window.ABCJS.edit.EditArea(editarea) : this.editarea = editarea, this.editarea.addSelectionListener(this), this.editarea.addChangeListener(this), params.canvas_id ? this.div = document.getElementById(params.canvas_id) : params.paper_id ? this.div = document.getElementById(params.paper_id) : (this.div = document.createElement("DIV"), this.editarea.getElem().parentNode.insertBefore(this.div, this.editarea.getElem())), params.generate_midi && (this.midiParams = params.midi_options || {}, this.midiParams.generateDownload && ("string" == typeof params.midi_download_id ? this.downloadMidi = document.getElementById(params.midi_download_id) : params.midi_download_id && (this.downloadMidi = params.midi_download_id)), this.midiParams.generateInline !== !1 && ("string" == typeof params.midi_id ? this.inlineMidi = document.getElementById(params.midi_id) : params.midi_id && (this.inlineMidi = params.midi_id))), (params.generate_warnings || params.warnings_id) && (params.warnings_id ? this.warningsdiv = document.getElementById(params.warnings_id) : this.warningsdiv = this.div), this.parserparams = params.parser_options || {}, this.onchangeCallback = params.onchange, this.engraverparams = params.render_options || {}, params.gui && (this.target = document.getElementById(editarea), this.engraverparams.editable = !0), this.oldt = "", this.bReentry = !1, this.parseABC(), this.modelChanged(), this.addClassName = function(element, className) {
            var hasClassName = function(element, className) {
                var elementClassName = element.className;
                return elementClassName.length > 0 && (elementClassName === className || new RegExp("(^|\\s)" + className + "(\\s|$)").test(elementClassName))
            };
            return hasClassName(element, className) || (element.className += (element.className ? " " : "") + className), element
        }, this.removeClassName = function(element, className) {
            return element.className = window.ABCJS.parse.strip(element.className.replace(new RegExp("(^|\\s+)" + className + "(\\s+|$)"), " ")), element
        }, this.setReadOnly = function(readOnly) {
            var readonlyClass = "abc_textarea_readonly",
                el = this.editarea.getElem();
            readOnly ? (el.setAttribute("readonly", "yes"), this.addClassName(el, readonlyClass)) : (el.removeAttribute("readonly"), this.removeClassName(el, readonlyClass))
        }
    }, window.ABCJS.Editor.prototype.renderTune = function(abc, params, div) {
        var tunebook = new ABCJS.TuneBook(abc),
            abcParser = window.ABCJS.parse.Parse();
        abcParser.parse(tunebook.tunes[0].abc, params);
        var tune = abcParser.getTune(),
            paper = Raphael(div, 800, 400),
            engraver_controller = new ABCJS.write.EngraverController(paper, this.engraverparams);
        ...
        if (!this.bReentry) {
            this.bReentry = !0, this.timerId = null, this.div.innerHTML = "";
            var paper = Raphael(this.div, 800, 400);
            this.engraver_controller = new ABCJS.write.EngraverController(paper, this.engraverparams), this.engraver_controller.engraveABC(this.tunes), this.tunes[0].engraver = this.engraver_controller;
            var downloadMidiHtml = "",
                inlineMidiHtml = "";
            if (this.midiParams && !this.midiPause) {
                for (var i = 0; i < this.tunes.length; i++) {
                    var midi = window.ABCJS.midi.create(this.tunes[i], this.midiParams);
                    this.midiParams.generateInline && this.midiParams.generateDownload ? (downloadMidiHtml += window.ABCJS.midi.generateMidiDownloadLink(this.tunes[i], this.midiParams, midi.download, i),
                        inlineMidiHtml += window.ABCJS.midi.generateMidiControls(this.tunes[i], this.midiParams, midi.inline, i)) : this.midiParams.generateInline ? inlineMidiHtml += window.ABCJS.midi.generateMidiControls(this.tunes[i], this.midiParams, midi, i) : downloadMidiHtml += window.ABCJS.midi.generateMidiDownloadLink(this.tunes[i], this.midiParams, midi, i)
                }
                this.midiParams.generateDownload && (this.downloadMidi ? this.downloadMidi.innerHTML = downloadMidiHtml : this.div.innerHTML += downloadMidiHtml);
                var find = function(element, cls) {
                    var els = element.getElementsByClassName(cls);
                    return 0 === els.length ? null : els[0]
                };
                if (this.midiParams.generateInline) {
                    var inlineDiv;
                    if (this.inlineMidi ? (this.inlineMidi.innerHTML = inlineMidiHtml, inlineDiv = this.inlineMidi) : (this.div.innerHTML += inlineMidiHtml, inlineDiv = this.div), this.midiParams.animate || this.midiParams.listener)
                        for (i = 0; i < this.tunes.length; i++) {
                            var parent = find(inlineDiv, "abcjs-midi-" + i);
                            parent.abcjsTune = this.tunes[i], parent.abcjsListener = this.midiParams.listener, parent.abcjsAnimate = this.midiParams.animate
                        }
                }
            }
            if (this.warningsdiv && (this.warningsdiv.innerHTML = this.warnings ? this.warnings.join("<br />") : "No errors"), this.target) {
                var textprinter = new window.ABCJS.transform.TextPrinter(this.target, !0);
                textprinter.printABC(this.tunes[0])
            }
            this.engraver_controller.addSelectListener(this), this.updateSelection(), this.bReentry = !1
        }
    }, window.ABCJS.Editor.prototype.paramChanged = function(engraverparams) {
        this.engraverparams = engraverparams, this.oldt = "", this.fireChanged()
    }, window.ABCJS.Editor.prototype.parseABC = function() {
        var t = this.editarea.getString();
        if (t === this.oldt) return this.updateSelection(), !1;
        if (this.oldt = t, "" === t) return this.tunes = void 0, this.warnings = "", !0;
        var tunebook = new ABCJS.TuneBook(t);
        this.tunes = [], this.startPos = [], this.warnings = [];
        for (var i = 0; i < tunebook.tunes.length; i++) {
            var abcParser = new window.ABCJS.parse.Parse;
            abcParser.parse(tunebook.tunes[i].abc, this.parserparams), this.tunes[i] = abcParser.getTune(), this.startPos[i] = tunebook.tunes[i].startPos;
            for (var warnings = abcParser.getWarnings() || [], j = 0; j < warnings.length; j++) this.warnings.push(warnings[j])
        }
        return !0
        ...
        if (void 0 !== this.indicate_changed) {
            var addClassName = function(element, className) {
                ...
                readonlyClass = "abc_textarea_dirty",
                el = this.editarea.getElem();
            isDirty ? addClassName(el, readonlyClass) : removeClassName(el, readonlyClass)
        }
        ...
        if (!this.bIsPaused && this.parseABC()) {
            ...
            this.wasDirty !== isDirty && (this.wasDirty = isDirty, this.setDirtyStyle(isDirty)), this.onchangeCallback && this.onchangeCallback(this)
        }
        ...
        var offset = void 0 !== tuneNumber ? this.startPos[tuneNumber] + tuneNumber : 0;
        ...
        this.bIsPaused = shouldPause, shouldPause || this.fireChanged()
